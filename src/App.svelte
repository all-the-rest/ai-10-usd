<script lang="ts">
  ...
  function resolveInitialDark(themeParam: string | null, stored: string | null): boolean {
  ...
  function setDarkValue(next: boolean) {
    dark = next;
    try {
      if (typeof localStorage !== "undefined") {
  ...
  onMount(() => {
    ...
    if (typeof window !== "undefined" && typeof window.matchMedia !== "undefined") {
      ...
      if (stored === null && params.get("theme") === null) {
        const mq = window.matchMedia("(prefers-color-scheme: dark)");
        const onChange = (e: MediaQueryListEvent) => {
          if (typeof localStorage !== "undefined" && localStorage.getItem("theme") !== null) return;
          dark = e.matches;
        };
  ...
  <header class="sticky top-0 z-10 bg-base-200 shadow-sm">
    ...
    <div class="navbar-end gap-2 text-sm text-base-content/70 sm:gap-3">
      ...
      <label class="swap swap-rotate">
        <input type="checkbox" class="theme-controller" value="dark" checked={dark} onchange={() => setDarkValue(!dark)} />
