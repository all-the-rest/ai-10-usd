            ...
            <tbody>
              {#each filteredRows as row}
                <tr class={rowClass(row)}>
                  ...
                  <td class="text-right number">{#if row.openCodeGo}<div><span class:font-bold={row.comparison.winner === "openCodeGo"} class:text-success={row.comparison.winner === "openCodeGo"}>{compact(row.openCodeGo.normalizedRequestsPer10)}</span><div class="text-xs font-normal text-base-content/45">{money(row.openCodeGo.averageAllowance, 0)} usage</div></div>{:else}<span class="text-base-content/35">-</span>{/if}</td>
