 ...
 * exportiert und ohne Nebenwirkungen (kein Network/IO) — damit sind sie direkt
 * unit-testbar, statt dass Tests die Logik nachbauen (Mirror-Tests).
 * `buildComparison` ist die komplette Pipeline vom Tracker-Snapshot bis zum
 * Ausgabe-Objekt (ohne Fetch und ohne `file:`-Schreiben).
 */

export const TARGET_PRICE = 10;
export const COMMAND_CODE_PLAN_ID = "goat";
...
export const COMMAND_CODE_PAID_PRICE = 10.77;
export const COMMAND_CODE_PAID_PRICE_SOURCE = "https://cc-pricing.all-the.rest/?sort=requests%3Adesc";
...
export const AVERAGE_MESSAGE_PATTERN = { input: 800, cachedRead: 50000, output: 162 };

// A difference below this threshold (relative to the worse plan) counts as a
// draw instead of a winner.
export const DRAW_THRESHOLD_PERCENT = 10;
...
export function isFree(model, provider) {
...
export function requestCost(model, pattern) {
...
export function commandCodeAllowance(model, plan) {
  return finite(model.allowances?.goat) ?? finite(plan.defaultAllowance) ?? finite(plan.creditsMonthly);
}

export function computeFinite(models, provider, paidMonthly, plan, pattern) {
  const values = models
    .map((model) => {
...
export function providerValue(models, provider, paidMonthly, plan, pattern) {
...
export function variantKind(tier) {
  if (!tier) return null;
...
export function variantTitle(kind) {
...
export function findOutliers(rows) {
...
/**
 * Kompletter Generator-Lauf über zwei Tracker-Snapshots (ohne Fetch/Schreiben).
 * Liefert exakt das Objekt, das `build-comparison.mjs` als `public/data/latest.json`
 * serialisiert.
 */
