  ...
  try {
    ...
    try {
      ...
      if (!res.ok) fail(`GET /data/latest.json → HTTP ${res.status}`);
        ...
        if (!data.sources?.openCodeGo?.url || !data.sources?.commandCode?.url) {
          fail("latest.json ohne sources.openCodeGo/commandCode");
        } else ok(`quellen: ${data.sources.openCodeGo.url}, ${data.sources.commandCode.url}`);
        if (!data.statistics || typeof data.statistics !== "object") fail("latest.json ohne statistics");
        else ok("statistics vorhanden");
      }
    } catch (e) {
